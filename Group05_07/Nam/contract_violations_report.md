...
### 6. Thay đổi định dạng lỗi (Error Response Format Change)
- **Mô tả**: Provider đổi format từ `{ "error": "msg" }` sang `{ "message": "msg", "code": 400 }`.
- **Endpoint bị ảnh hưởng**: Các endpoint có trả về lỗi (ví dụ `POST /api/cart`)
- **Loại violation**: Error Response Format Change
- **Cách phát hiện**: Pact verification output báo lỗi thiếu trường (BodyMismatch).
- **Mức độ nghiêm trọng**: High (Consumer không đọc được chi tiết lỗi, có thể gây crash hoặc UX kém).

---

### 7. Xóa endpoint hoặc thay đổi URL/HTTP Method
- **Mô tả**: Xóa endpoint hoặc thay đổi URL/HTTP Method (Ví dụ chuyển từ `POST /api/cart` sang `PUT /api/cart` hoặc đổi path).
- **Endpoint bị ảnh hưởng**: `POST /api/cart`
- **Loại violation**: Missing endpoint / Method mismatch
- **Cách phát hiện**: Pact verification output báo lỗi HTTP status 404 Not Found hoặc 405 Method Not Allowed.
- **Mức độ nghiêm trọng**: Critical (API hoàn toàn không truy cập được).
