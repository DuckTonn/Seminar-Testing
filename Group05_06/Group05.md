...
**Prompt đã sử dụng**
> - Hướng dẫn tôi các bước tích hợp Newman API Test và Pact Contract Testing vào quy trình CI/CD.
> - Viết file newman.yml để tự động chạy Newman (có xuất HTML Report) và chạy tiếp Pact Verification kể cả khi test fail.


**AI đã thực hiện**

- Cấu trúc lại file workflow `.github/workflows/newman.yml` để cài đặt dependencies và chạy song song Newman lẫn Pact.
- Cấu hình upload thư mục `reports/` chứa file HTML report của Newman và file logs của Pact lên GitHub Artifacts với cấu hình `if: always()`.


**Sinh viên đã thực hiện**

- Phân tích và lên ý tưởng tích hợp CI/CD tự động hóa kiểm thử giữa Newman và Pact.
- Soạn prompt chi tiết để AI xây dựng khung workflow phù hợp với cấu trúc dự án.
- Áp dụng các thay đổi vào file `.github/workflows/newman.yml` trên nhánh chính.
- Tải về và kiểm chứng chất lượng các tệp báo cáo `newman-report.html` và `pact-verification-report.txt` trong phần Artifacts để đảm bảo minh chứng đạt yêu cầu.

**Minh chứng**

- `.github/workflows/newman.yml`
- Báo cáo Newman HTML: [newman-report.html](./Luân/newman-report.html)
- Báo cáo Pact log: [pact-verification-report.txt](./Luân/pact-verification-report.txt)
...
- **Access time**: 18:30 ngày 17/07/2026.
- **Prompts used**:
  - "Hướng dẫn tôi các bước tích hợp Newman API Test và Pact Contract Testing vào quy trình CI/CD."
  - "Viết file newman.yml để tự động chạy Newman (có xuất HTML Report) và chạy tiếp Pact Verification kể cả khi test fail."
- **Purpose of use**: Hỗ trợ tham khảo cú pháp thiết lập workflow CI/CD và định dạng khung báo cáo tuần.
- **Which content was generated by AI**: Đoạn cấu hình YAML trong file `.github/workflows/newman.yml`.
- **Which content was done independently and how the student edited or validated it**: Sinh viên tự rà soát lại các đường dẫn thư mục, chỉnh sửa logic cài đặt thư viện kiểm thử, push code kiểm tra pipeline trên GitHub Actions thực tế và tự tổng hợp, đưa các tệp minh chứng vào thư mục báo cáo của nhóm.

# Tasks Planned for Next Week

- Hoàn thành dự thảo tài liệu hướng dẫn sử dụng (User Guide) cho cả 3 mảng: Postman API Testing, AI Testing và Pact Contract Testing.
- Tổng hợp phân tích Failure Modes của cả 3 phần kiểm thử thành một mục chung trong User Guide.
- Chuẩn bị nội dung Slide Seminar nháp (các thành viên tự thiết kế phần slide cá nhân rồi ghép lại).
- Lên kịch bản và quay thử video demo (phối hợp kiểm thử Postman API và kiểm thử AI).
- Hoàn thiện sơ đồ Pipeline Diagram mô tả luồng CI/CD và viết hướng dẫn chạy.
- Thực hiện và hoàn thiện Activity Worksheet của nhóm.
